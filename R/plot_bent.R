...
#' plot_bent(df_bend,df)
plot_bent = function(bent, unbent = NULL, ...){
  ...
  side_view =
    ggplot2::ggplot(data = df, ggplot2::aes(x = x, y = z, color = type))+
    ggplot2::geom_line()+
  ...
  top_view =
    ggplot2::ggplot(data = df, ggplot2::aes(x = x, y = y, color = type))+
    ggplot2::geom_line()+
    ggplot2::coord_cartesian(xlim = plot_range,
                             ylim = c(-plot_range_max,plot_range_max))+
...
#' @rdname plot_bent
#' @export
plot_bent_3d = function(bent, unbent = NULL, ...){
  plot_range = range(bent$x,bent$y,bent$z,unbent$x,unbent$y,unbent$z)
  plot_range_max = max(plot_range)

  plot_ly(bent, x= ~x, y = ~y, z= ~z, type = 'scatter3d',
          mode = 'lines', name = "Bent")%>%
    {
      if(!is.null(unbent)){
        add_trace(p = ., data = unbent, x = ~x, y = ~y, z = ~z,
                  name = "Initial state")
      }else{
        .
      }
